...
void Game::playGame(Player& player) {
	...
	int correctAnswers = 0; //numaram raspunsurile corecte ca sa ne oprim la 15
	...
	while (true) {
		...
		if (!actualQuestions.empty()) {
			...
			cout << "Nu sunt intrebari disponibile din acesst domeniu." << endl;
			continue; 
		}
		...
		for (Question ques : actualQuestions) {
			...
			if (ques.checkAnswer(userAnswer)) {
				...
				correctAnswers = 0;
				char continueInput;
				cout << "Vrei sa continui? Apasa 'e' pentru a iesi sau orice alta tasta pentru a incepe o noua runda" << endl;
				cin >> continueInput;
